...
router.post("/update/mail", (req, res) => {
    ...
    utils.executeQuery(query, [mail1, user_id], (err, result) => {
        if (err) {
            res.status(500).json({errorCause: 'Une erreur est survenue'})
        } else {
            res.status(200).json({status: 'success', msg: 'Adresse mail mise à jour'})
        }
    });
});
...
router.post('/update/password', (req, res) => {
    //Variables pour récupérer les paramètres
       ...
       if (password2 != password3) {
        res.status(400).json({status:'error', cause: 'Les mots de passe doivent être identiques'});
        return;
    }
     ...
     utils.executeQuery(query, [password2, user_id], (err, result) => {
         if (err) {
            res.status(500).json({errorCause: 'Erreur mot de passe'})
         } else {
